...
# | 비용 | 없음 | LLM 호출 비용 발생 |
#
# 이 파일은 외부 API를 Mock 데이터로 대체하여 로컬에서 실행 가능합니다.
...
if not api_key:
    raise EnvironmentError(
        ...
        "프로젝트 루트의 .env 파일에 아래와 같이 입력하세요:\n\n"
        "  OPENAI_API_KEY=sk-..."
    )

print(f"API 키 로드 완료: {api_key[:8]}...{api_key[-4:]}")

# ▶ 방법 A: 사내 Private LLM (workflow.py 환경)
...
# ▶ 방법 B: OpenAI (기본, .env에서 키 자동 로드)
llm = ChatOpenAI(model="gpt-4o-mini", temperature=0, api_key=api_key)
llm_with_tools = llm.bind_tools(TOOLS)
print(f"LLM 초기화 완료: {llm.model_name}")
print(f"bind_tools 완료: {len(TOOLS)}개 도구 등록")
...
# ── 7. Mock LLM — API 키 없이 도구 선택 로직 확인 ──────────────────────────────────────────────────
MOCK_TOOL_DECISIONS = [
...
# ── 8. 시나리오 테스트 ──────────────────────────────────────────────────
def run_scenarios():
    """실제 LLM API 호출 시나리오 테스트 (API 키 필요)"""
    scenarios = [
        "AB123456 슬롯 정보 알려줘",
        "AB123456 계측 지시 확인해줘",
        "AB123456 퓨처홀드 걸려있어?",
        "M15A001 지금 돌아가?",
        "M15에서 CVD 장비 몇 대 다운이야?",
        "AB1 CVD 증착 이전 공정 뭐야?",
        "CVD 공정에서 두께 편차가 생기는 원인이 뭐야?",
    ]
    for q in scenarios:
        print(f"\n{'='*60}")
        state = run_tool_calling(q)
...
if __name__ == "__main__":
    # Mock 시뮬레이션 (API 키 불필요)
    run_mock_simulation()

    # 실제 LLM 시나리오 (API 키 필요 시 주석 해제)
    # run_scenarios()
